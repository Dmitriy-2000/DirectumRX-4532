...
namespace Sungero.Docflow
{
  ...
  partial class ApprovalTaskSharedHandlers
  {
    ...
    public virtual void DocumentGroupAdded(Sungero.Workflow.Interfaces.AttachmentAddedEventArgs e)
    {
      ...
      var availableApprovalRules = Functions.OfficialDocument.Remote.GetApprovalRules(document).ToList();
      if (availableApprovalRules.Any())
      {
        ...
        if (defaultApprovalRule.Count() == 1)
          _obj.ApprovalRule = defaultApprovalRule.First();
        else
          if (_obj.ApprovalRule != null && !defaultApprovalRule.Contains(_obj.ApprovalRule))
